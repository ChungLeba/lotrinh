...
/* Function , variable*/
  ...
  /* Đếm lượt views tuyến */
  async function demluotview_lotrinh(idtuyen,iduser){
    /* Địa điểm */     
    let demluotxem_lotrinh = await routerModel.updateOne({
        _id: idtuyen
      },{
        $inc: {totalviews:1}
      })
...
router.get('/lo-trinh/lich-trinh/chieu-di/:id/chuyen/:machuyen',function(req , res, next){
  ...
  .then(data=>{
      ...
      //Đếm views
      demluotview_lotrinh(data[0]._id,data[0].partnerID._id)
      .then(data=>{
        //console.log(data)
      })
      .catch(err=>{
